      ...
      <div className="h-full flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1, x: isPassError ? [0, -10, 10, -10, 10, 0] : 0 }} className="w-full max-w-md bg-[#09090b] border border-zinc-800 rounded-3xl p-8 shadow-2xl relative z-10">
          <div className="flex flex-col items-center mb-8">
            <div className={`p-4 rounded-full mb-4 border ${isPassError ? 'bg-destructive/10 text-destructive border-destructive/20' : 'bg-primary/10 text-primary border-primary/20'}`}><Lock size={32} /></div>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            <input type="password" placeholder="Senha de Acesso" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className={`w-full bg-zinc-900 border rounded-xl py-4 text-center text-white tracking-[0.5em] outline-none transition-all ${isPassError ? 'border-destructive' : 'border-zinc-800 focus:border-primary'}`} />
            <button type="submit" className="w-full py-4 rounded-xl bg-primary text-black font-black uppercase tracking-widest text-xs hover:bg-white transition-all flex items-center justify-center gap-2">Acessar CRM <ArrowRight size={16} /></button>
          </form>
        </motion.div>
      </div>
    );
