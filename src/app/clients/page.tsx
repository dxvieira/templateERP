...
import { 
  ...
  ChevronRight, FileBadge, Lock, ShieldCheck, ArrowRight, MapPin, Smartphone, MessageCircle, Loader2 
} from 'lucide-react';
...
function ClientsContent() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setDoc(clientRef, payload, { merge: true })
      .then(() => {
        toast({ title: editingClient ? "Cadastro Atualizado" : "Novo Cliente Registrado" });
        closeModal();
      })
      .catch((err) => {
    ...
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <motion.div 
          ...
          className="w-full max-w-md bg-[#09090b] border border-zinc-800 rounded-3xl p-8 shadow-2xl relative z-10"
        >
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-zinc-500 text-sm mt-2 text-center uppercase tracking-widest font-bold">Base de Dados Industrial</p>
          </div>
          <form onSubmit={handleUnlock} className="space-y-4">
            <input 
              ...
              className={`w-full bg-zinc-900 border rounded-xl py-4 text-center text-white tracking-[0.5em] outline-none transition-all ${isPassError ? 'border-destructive' : 'border-zinc-800 focus:border-primary'}`} 
            />
            <button type="submit" className="w-full py-4 rounded-xl bg-primary text-black font-black uppercase tracking-widest text-xs hover:bg-white transition-all flex items-center justify-center gap-2 group">
              Acessar CRM <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </form>
        </motion.div>
      </div>
    ...
    <div className="min-h-screen bg-[#050505] flex flex-col md:flex-row overflow-x-hidden selection:bg-primary selection:text-black">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pb-24 space-y-8 mt-16 md:mt-0">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
             ...
             <h1 className="text-4xl font-black text-white tracking-tight uppercase">
               Base de <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-orange-600">Parceiros</span>
             </h1>
          </div>
          <button onClick={() => openModal()} className="group flex items-center gap-2 px-6 py-3 rounded-full bg-primary text-black font-black uppercase tracking-widest text-[10px] hover:bg-white transition-all shadow-[0_0_20px_rgba(255,95,31,0.4)]">
        ...
        <div className="relative group max-w-2xl">
          ...
          <input 
            type="text" 
            placeholder="Buscar Cliente, Empresa ou Documento..." 
            value={searchTerm} 
            onChange={(e) => setSearchTerm(e.target.value)} 
            className="w-full bg-[#09090b] border border-zinc-800 rounded-2xl py-4 pl-12 pr-4 text-white placeholder-zinc-600 outline-none transition-all focus:border-primary/50 focus:shadow-[0_0_30px_-5px_rgba(255,95,31,0.15)]" 
          />
        </div>
              ...
              <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} key={client.id} onClick={() => openModal(client)} className="group relative bg-[#09090b] border border-zinc-800 rounded-2xl p-5 cursor-pointer transition-all duration-300 hover:border-primary/40 hover:bg-zinc-900/40 hover:-translate-y-0.5">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="shrink-0 w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-primary font-black text-lg">
                ...
                <div className="space-y-1.5">
                   <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400">
                     <Smartphone size={12} className="text-zinc-600 group-hover:text-primary transition-colors" />
                     <span>{client.mobile || client.landline || 'Sem contato'}</span>
                   </div>
                   <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-400">
                     <Mail size={12} className="text-zinc-600 group-hover:text-primary transition-colors" />
              ...
              <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-4xl bg-[#09090b] border border-zinc-800 rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8 bg-[#050505]">
                  <form id="clientForm" onSubmit={handleSave} className="space-y-8">
                    <section>
                      <h3 className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-2 border-b border-white/5 pb-2">
                         <FileBadge size={14} /> Dados Fiscais
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="lg:col-span-2">
                          <label className={labelClass}>Razão Social / Fantasia</label>
                          <input className={inputClass} value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} />
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <input className={inputClass} value={formData.stateInscription} onChange={e => setFormData({...formData, stateInscription: e.target.value})} />
                        </div>
                      </div>
                    </section>
                    ...
                    <section>
                      <h3 className="text-[10px] text-primary font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-2 border-b border-white/5 pb-2">
                         <Smartphone size={14} /> Contato
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         <div>
                            <label className={labelClass}>Telefone Fixo</label>
                            <input className={inputClass} value={formData.landline} onChange={e => setFormData({...formData, landline: e.target.value})} />
                         </div>
                         <div>
                            <label className={labelClass}>Celular / WhatsApp</label>
                            <div className="flex gap-2">
                               <input className={`${inputClass} flex-1`} value={formData.mobile} onChange={e => setFormData({...formData, mobile: e.target.value})} />
                               {formData.mobile && (
                    ...
                    <section>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                         <div>
                            ...
                            <input className={inputClass} value={formData.zip} onChange={e => setFormData({...formData, zip: e.target.value})} />
                         </div>
                         <div className="md:col-span-2">
                            <label className={labelClass}>Rua / Avenida</label>
                            <input className={inputClass} value={formData.street} onChange={e => setFormData({...formData, street: e.target.value})} />
                         </div>
                         <div>
                         ...
                         <div className="md:col-span-2">
                            ...
                            <input className={inputClass} value={formData.complement} onChange={e => setFormData({...formData, complement: e.target.value})} />
                         </div>
                      </div>
                    </section>
                  </form>
                ...
                <div className="p-6 border-t border-zinc-800 bg-zinc-900/30 flex justify-between items-center gap-4">
                  {editingClient && (
                    <button type="button" onClick={() => handleDelete(editingClient.id)} className="flex items-center gap-2 px-4 py-3 rounded-xl bg-destructive/10 text-destructive hover:bg-destructive hover:text-white transition-colors font-black text-[10px] uppercase tracking-widest"><Trash2 size={16} /> Excluir</button>
                  )} 
                  <div className="flex gap-3 w-full sm:w-auto ml-auto">
                     ...
                     <button form="clientForm" type="submit" disabled={isSubmitting} className="px-10 py-3 rounded-xl bg-primary text-black hover:bg-white transition-all font-black text-[10px] uppercase tracking-widest shadow-[0_0_20px_rgba(255,95,31,0.4)] disabled:opacity-50 flex items-center gap-2">
