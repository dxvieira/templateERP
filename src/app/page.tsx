...
const DashboardHeader = memo(({ onNewOrder }: { onNewOrder: () => void }) => (
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
    ...
    >
      ...
      <p className="text-muted-foreground text-[8px] md:text-[10px] uppercase tracking-[0.4em] font-medium">Status da Operação em Tempo Real</p>
    </motion.div>
    ...
    >
      <Button 
        onClick={onNewOrder}
        className="w-full md:w-auto bg-primary text-black font-black uppercase tracking-widest px-8 h-12 md:h-14 rounded-2xl hover:shadow-[0_0_30px_rgba(255,95,31,0.6)] hover:bg-primary transition-all gap-3 active:scale-95 transition-transform"
      >
    ...
    <div className="min-h-screen bg-[#0A0A0A] flex flex-col md:flex-row">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 space-y-6 md:space-y-8 mt-16 md:mt-0">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          <div className="lg:col-span-8 order-2 lg:order-1 space-y-6">
            <Card className="glass-card border-none bg-black/20">
              <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 pb-4 md:pb-6">
                <CardTitle className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">Protocolos Ativos</CardTitle>
                <div className="flex items-center gap-3">
                   <div className="w-2 h-2 rounded-full bg-primary/40 shadow-[0_0_10px_#FF5F1F] animate-pulse" />
                   <span className="text-[9px] text-primary/60 font-black uppercase tracking-widest hidden sm:inline">
          ...
          <div className="lg:col-span-4 order-1 lg:order-2 space-y-6 md:gap-8">
        ...
        <footer className="pt-8 pb-4 border-t border-white/5">
          <p className="text-[7px] md:text-[9px] text-muted-foreground/30 uppercase tracking-[0.5em] text-center">
