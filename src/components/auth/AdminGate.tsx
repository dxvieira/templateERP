...
import { Lock, ArrowRight, ShieldCheck, KeyRound, Loader2, X } from 'lucide-react';
...
/**
 * AdminGate - Nível 2 de Segurança Industrial.
 * Protege rotas sensíveis exigindo a Senha Mestre local.
 * Persistência via sessionStorage (apenas para a aba atual).
 */
export function AdminGate({ children }: AdminGateProps) {
  ...
  // Senha Mestre fixa conforme política industrial
  ...
  const handleUnlock = (e: React.FormEvent) => {
    ...
    if (passwordInput === MASTER_PASS) {
      ...
      toast({ title: "Privilégios Elevados", description: "Acesso administrativo concedido nesta sessão." });
    } else {
      setIsPassError(true);
      toast({ 
        variant: "destructive", 
    ...
    <div className="h-full flex items-center justify-center p-4 relative overflow-hidden bg-[#0A0A0A]">
      ...
      >
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Área Sensível</h2>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-center gap-2 opacity-30">
           ...
           <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">Admin Gate Ativo</span>
        </div>
      </motion.div>
    </div>
  );
