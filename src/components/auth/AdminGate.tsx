...
import { Lock, ArrowRight, ShieldCheck, X, AlertTriangle, KeyRound, Loader2 } from 'lucide-react';
...
/**
 * AdminGate - Nível 2 de Segurança.
 * Protege rotas sensíveis exigindo a Senha Mestre industrial.
...
export function AdminGate({ children }: AdminGateProps) {
  ...
  // Senha Mestre fixa conforme solicitado
  const MASTER_PASS = "@impactoADM.2026@";

  useEffect(() => {
    setIsMounted(true);
    // Verifica se já foi desbloqueado nesta aba
  ...
  const handleUnlock = (e: React.FormEvent) => {
    ...
    if (passwordInput === MASTER_PASS) {
      ...
      toast({ title: "Elevação de Privilégios", description: "Acesso administrativo concedido." });
    } else {
      setIsPassError(true);
      toast({ 
        variant: "destructive", 
    ...
    <div className="h-full flex items-center justify-center p-4 relative overflow-hidden bg-[#0A0A0A]">
      ...
      >
        <div className="flex flex-col items-center mb-8 text-center">
          <div className={`p-4 rounded-3xl mb-6 border transition-all duration-500 shadow-2xl ${isPassError ? 'bg-destructive/10 text-destructive border-destructive/30' : 'bg-primary/10 text-primary border-primary/30'}`}>
            <KeyRound size={32} className={isPassError ? 'animate-bounce' : ''} />
          </div>
          <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Área Restrita</h2>
          <p className="text-zinc-500 text-[9px] mt-2 uppercase tracking-[0.3em] font-bold leading-relaxed max-w-[280px]">
            Insira a Senha Mestre para gerenciar dados financeiros e pauta industrial.
          </p>
        </div>
        ...
        <form onSubmit={handleUnlock} className="space-y-4">
          <div className="relative group">
            <input 
              ...
              className={`w-full bg-zinc-950 border rounded-2xl py-5 text-center text-white tracking-[0.5em] outline-none transition-all duration-300 font-black placeholder:text-zinc-800 placeholder:tracking-widest ${isPassError ? 'border-destructive/50 ring-2 ring-destructive/10' : 'border-zinc-800 focus:border-primary/50'}`} 
            />
          </div>
          <Button 
            type="submit" 
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-center gap-2 opacity-30">
           ...
           <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">Admin Gate Active</span>
        </div>
      </motion.div>
    </div>
  );
