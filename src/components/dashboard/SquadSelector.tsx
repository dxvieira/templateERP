...
export function SquadSelector({ order, isOpen, onClose }: SquadSelectorProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-background/95 backdrop-blur-md">
        <motion.div
          initial={{ scale: 0.95, opacity: 0, y: 15 }}
          animate={{ scale: 1, opacity: 1, y: 0 }}
          exit={{ scale: 0.95, opacity: 0, y: 15 }}
          transition={{ type: 'spring', stiffness: 350, damping: 28 }}
          className="bg-card w-full max-w-3xl border border-border rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col max-h-[90vh] overflow-hidden relative"
        >
          {/* ── Background Glow ──────────────────────────────────────── */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[100px] bg-primary/10 blur-[100px] pointer-events-none" />

          {/* ── Header ────────────────────────────────────────────── */}
          <div className="p-8 border-b border-border bg-secondary/20 backdrop-blur-xl flex items-center justify-between relative z-10">
            <div className="flex items-center gap-6">
              ...
              <div className="min-w-0">
                <h2 className="text-2xl font-black text-foreground uppercase tracking-tighter leading-tight">
                  Definição de Equipe
                </h2>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest truncate">
                    • {order.client}
                  </span>
                  <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-secondary text-primary border border-primary/20 ml-2">
                    {order.status}
                  </span>
                </div>
              </div>
            </div>
            <button onClick={onClose} className="p-3 text-muted-foreground hover:text-foreground bg-secondary hover:bg-secondary rounded-full transition-all border border-border shrink-0 ml-4">
              <X size={24} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-border pb-2">
                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">Operadores e Liderança ({allEmployees.length})</p>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected
                          ? 'border-white/20 bg-secondary shadow-lg scale-[1.02]'
                          : 'border-border bg-secondary hover:border-border hover:bg-accent'
                      )}
                    >
                      {/* Avatar circle */}
                      <div
                      ...
                      <div className="flex flex-col items-start min-w-0">
                        <span className={cn("text-[11px] font-black uppercase tracking-wide truncate w-full text-left transition-colors", isSelected ? "text-foreground" : "text-muted-foreground group-hover:text-foreground")}>
                          {emp.name}
                        </span>
                      </div>
          ...
          {/* ── Footer ───────────────────────────────────────────── */}
          <div className="p-8 border-t border-border bg-secondary/40 flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10 backdrop-blur-md">
            
            {/* Live Preview of Squad */}
            <div className="flex flex-col items-center sm:items-start w-full sm:w-auto">
              {selectedIds.length > 0 ? (
                <>
                  <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-2 text-center sm:text-left">Squad Configurado</p>
                  <div className="flex items-center gap-3 bg-card px-4 py-2 rounded-2xl border border-border shadow-inner">
            ...
            <div className="flex w-full sm:w-auto gap-3">
              <button
                onClick={onClose}
                className="py-4 px-6 rounded-2xl border border-border text-muted-foreground font-black uppercase text-[10px] tracking-widest hover:bg-secondary hover:text-foreground transition-all min-w-[120px]"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 sm:flex-none py-4 px-8 rounded-2xl bg-primary text-primary-foreground font-black uppercase text-[11px] tracking-widest hover:bg-white transition-all shadow-[0_0_30px_rgba(255,95,31,0.3)] hover:shadow-[0_0_40px_rgba(255,95,31,0.5)] disabled:opacity-30 disabled:shadow-none flex items-center justify-center gap-2 active:scale-95"
              >
