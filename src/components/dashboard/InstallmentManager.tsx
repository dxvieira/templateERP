  ...
  const handleSettle = async (inst: Installment, method: PaymentMethod) => {
      ...
      const updated = installments.map(i => {
        if (i.index === inst.index) {
          return {
            ...
            paid_by: user?.email || 'sistema@exemplo.com'
          };
        }
        return i;
      });
    ...
    try {
      await settleInstallment(
        ...
        inst.index, 
        method, 
        user.email || 'operacional@exemplo.com',
        installments
      );
    ...
    <div className="space-y-4">
      ...
      {!readOnly && (
        <div className="border border-border rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between p-1 pr-4 bg-secondary/20">
            <button
              type="button"
              onClick={() => setShowGenerator(!showGenerator)}
              className="flex-1 flex items-center justify-between p-3 text-left hover:bg-secondary/50 transition-colors"
            >
              ...
              <ChevronDown size={14} className={cn("text-muted-foreground transition-transform", showGenerator && "rotate-180")} />
            </button>
            ...
            {installments.length > 0 && (
              <button
                ...
                className={cn(
                  ...
                  confirmClear 
                    ? "bg-red-600 text-foreground animate-pulse" 
              ...
              >
                <div className="p-4 pt-0 space-y-4 border-t border-border">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[9px] text-muted-foreground font-black uppercase tracking-widest block mb-1">Nº de Parcelas</label>
                      <input
                        type="number"
                        min={1}
                        max={36}
                        ...
                        className="w-full bg-secondary border border-border rounded-xl p-3 text-foreground text-sm font-black text-center outline-none focus:border-primary/50"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] text-muted-foreground font-black uppercase tracking-widest block mb-1">Data da 1ª Parcela</label>
                      <input
                        type="date"
                        value={startDate}
                        onChange={e => setStartDate(e.target.value)}
                        className="w-full bg-secondary border border-border rounded-xl p-3 text-foreground text-sm outline-none focus:border-primary/50"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={cn(
                        ...
                        hasEntry ? "bg-primary/10 border-primary/30 text-primary" : "border-border text-muted-foreground hover:border-zinc-600"
                      )}
                    >
                      <DollarSign size={12} />
                  ...
                  <AnimatePresence>
                    {hasEntry && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
                        <label className="text-[9px] text-muted-foreground font-black uppercase tracking-widest block mb-1">Valor da Entrada (R$)</label>
                        <input
                          type="number"
                          step="0.01"
                          min={0}
                          ...
                          className="w-full bg-secondary border border-border rounded-xl p-3 text-primary text-sm font-black outline-none focus:border-primary/50"
                          placeholder="0.00"
                        />
                      </motion.div>
                    )}
                  ...
                  <div className="bg-secondary/50 border border-border rounded-xl p-3 text-center">
                    <p className="text-[9px] text-muted-foreground uppercase font-black">Prévia do Rateio</p>
                    <p className="text-foreground font-black font-mono mt-1">
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-black uppercase text-[10px] tracking-widest hover:bg-white transition-all flex items-center justify-center gap-2"
                  >
      ...
      <div className="space-y-2">
        {installments.length > 0 ? (
          ...
          <div className="text-center py-12 border border-dashed border-border rounded-3xl bg-secondary/10">
            <CreditCard size={32} className="mx-auto mb-3 text-muted-foreground opacity-30" />
            <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Nenhuma fatura lançada</p>
            <p className="text-[9px] text-muted-foreground mt-1">Use o Gerador acima para criar o cronograma de pagamento</p>
          </div>
        )}
      </div>
      ...
      {!readOnly && installments.length > 0 && (
        <button
          ...
          className="w-full py-3 rounded-2xl border border-dashed border-border text-muted-foreground text-[10px] font-black uppercase tracking-widest hover:border-primary hover:text-primary transition-all flex items-center justify-center gap-2"
        >
        ...
        <div className="flex items-center gap-2 shrink-0">
          {statusKey !== 'paid' && !readOnly && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-primary-foreground transition-all"
            >
        ...
        {isOpen && statusKey !== 'paid' && (
          ...
          >
            <div className="p-4 space-y-3">
              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Forma de Pagamento</p>
              ...
              <button
                ...
                disabled={isProcessing}
                className="w-full py-3 rounded-xl bg-emerald-600 text-foreground font-black text-[10px] uppercase tracking-widest hover:bg-emerald-500 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
