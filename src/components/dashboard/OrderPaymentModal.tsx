...
import { 
  ...
  DollarSign, Plus, ArrowRight, Wallet, History 
} from 'lucide-react';
...
export function OrderPaymentModal({ order, isOpen, onClose }: OrderPaymentModalProps) {
  const financialSummary = useMemo(() => {
    ...
    const total = Number(order.calculatedTotal || 0);
    const paid = Number(order.calculatedPaid || 0);
    const balance = total - paid;
    const installments = Array.isArray(order.installments) ? order.installments : [];
    
    const today = startOfDay(new Date());

    const processedInstallments = installments.map((inst: any, index: number) => {
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-[#050505] space-y-6">
              ...
              <div className="space-y-3">
                {financialSummary.processedInstallments.length > 0 ? (
                  financialSummary.processedInstallments.map((inst) => (
                    <div 
                      key={inst.index}
                      className={cn(
                    ...
                    >
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-6 w-full sm:w-auto justify-between sm:justify-end">
                          ...
                          <div className={cn(
                            "flex items-center gap-1.5 px-3 py-1 rounded-full border text-[8px] font-black uppercase tracking-widest shadow-sm",
                            inst.isPaid ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : 
                            inst.isOverdue ? "bg-red-500/10 border-red-500/20 text-red-500" : 
                            "bg-orange-500/10 border-orange-500/20 text-orange-500"
                          )}>
                            {inst.isPaid ? <CheckCircle2 size={10}/> : inst.isOverdue ? <AlertTriangle size={10}/> : <Clock size={10}/>}
                            {inst.isPaid ? 'Liquidado' : inst.isOverdue ? 'Atrasado' : 'Pendente'}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="p-8 border-t border-zinc-800 bg-zinc-900/50">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button 
                    className="flex-1 sm:flex-none px-8 py-4 rounded-2xl bg-primary text-black font-black uppercase text-[10px] tracking-widest shadow-[0_0_30px_rgba(255,95,31,0.4)] hover:bg-white hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2"
                  >
